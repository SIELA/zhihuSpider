...
class IPPOOLS(HttpProxyMiddleware):
    ...
    def __init__(self,ip=''):
        ...
        self.IP_POOLS=list(collection.find({'speed':{'$lt':1}},{'ip':1,'port':1,}))
        self.ip=random.choice(self.IP_POOLS)
    def process_request(self, request, spider):
        #使用代理ip
        ip=random.choice(self.IP_POOLS) #随机选择一个ip
        print 'Using IP.address:'+str(self.ip['ip'])
        try:
            request.meta["proxy"]="http://"+str(self.ip['ip'])+':'+str(self.ip['port'])
        except Exception,e:
            print e
            pass
'''
